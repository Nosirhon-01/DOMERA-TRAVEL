...
import { MapPin, ArrowUpRight, Compass, Send } from 'lucide-react';
          ...
          {t.destinations.items.map((dest, idx) => (
            <motion.div
              ...
              className={`group relative rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 ${
                dest.featured ? 'lg:col-span-2 h-[440px]' : 'h-[440px]'
              }`}
            >
              {/* Background Image */}
              <img 
              ...
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#082A5B]/95 via-[#082A5B]/40 to-transparent group-hover:from-[#082A5B]/98 transition-colors duration-500" />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-10 text-white flex flex-col justify-end">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-extrabold mb-1 text-white group-hover:text-[#4FC3F7] transition-colors">
                      {dest.name}
                    </h3>
                    <p className="text-xs sm:text-sm text-slate-200/90 line-clamp-2 max-w-lg leading-relaxed font-normal">
                      {dest.shortDesc}
                    </p>
                  </div>
                </div>

                {/* Dual Action Buttons: [ Batafsil ] [ Bron qilish ] */}
                <div className="mt-2 pt-3 border-t border-white/15 flex items-center space-x-3">
                  <button
                    onClick={() => setSelectedDestination(dest)}
                    className="flex-1 py-2.5 px-4 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-bold transition-all border border-white/20 flex items-center justify-center space-x-1.5"
                  >
                    <span>Batafsil</span>
                  ...
                  <button
                    onClick={() => setBookingDestination(dest.name)}
                    className="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-[#1565FF] to-[#082A5B] hover:from-[#082A5B] hover:to-[#1565FF] text-white text-xs font-bold transition-all shadow-md flex items-center justify-center space-x-1.5"
                  >
